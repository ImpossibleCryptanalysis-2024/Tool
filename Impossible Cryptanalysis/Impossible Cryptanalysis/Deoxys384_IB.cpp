...
void keySchedule(GRBModel& model, vector<vector<vector<GRBVar>>>& roundkey) {
	...
	// 
	// counter for s0|s1=11 and k=1 in ROUND+1 rounds：0,1,2,ROUND+1
	for (int i = 0; i < state; i++) {
		GRBQuadExpr KS1 = 0;
		int j = i;
		for (int r = 0; r < ROUND + 1; r++) {
...
int main() {
	try {
		...
		//model.addConstr(Up_StateW[2][0][2] == 1);
		//model.addConstr(Lo_StateX[10][0][2] == 1);
