...
void KeyBridge(GRBModel& model, vector<vector<vector<GRBVar>>>& Up_Roundkey, vector<vector<vector<GRBVar>>>& Lo_Roundkey, GRBQuadExpr& key_involved_independent, vector<GRBVar>& independent) {
	...
	for (int i = 0; i < state; i++) {
		...
		model.addGenConstrIndicator(Ind, 0, cnt <= 0);
		model.addGenConstrIndicator(Ind, 1, cnt >= 1);

		independent[i] = model.addVar(0.0, GRB_INFINITY, 0.0, GRB_INTEGER);
		model.addConstr(independent[i] == cnt);

		key_involved_independent += (Ind * 1 + (1 - Ind) * cnt);
	}
}
...
int main() {
	try {
		...
		////model.addConstr(Lo_StateX[12][0][2] == 1);
		//// rtk
		//model.addConstr(Up_StateY[4][0][2] == 1);
		//model.addConstr(Lo_StateX[13][0][2] == 1);
		...
		for (int i = 0; i < state; i++) {
			obj1 += Up_StateX[1][i][1] + Lo_StateW[ROUND - 1][i][1];
		}
		...
		model.addQConstr(obj4 >= -15);
		model.addConstr(obj2 >= -15);
